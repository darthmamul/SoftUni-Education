namespace BachSoft.IO
{
    ...
    public class CommandInterpreter : IInterpreter
    {
        ...
        private IExecutable ParseCommand(string input, string command, string[] data)
        {
            ...
            Command execute = (Command)Activator.CreateInstance(typeOfCommand, parametersForConstruction);
            ...
            foreach (var fieldOfCommand in fieldsOfCommand)
            {
                ...
                if (injAtr != null)
                {
                    if (fieldsOfInterpreter.Any(x => x.FieldType == fieldOfCommand.FieldType))
                        {
                        fieldOfCommand.SetValue(execute, fieldsOfInterpreter.First(x => x.FieldType == fieldOfCommand.FieldType).GetValue(this));
                    }
                }
            }
