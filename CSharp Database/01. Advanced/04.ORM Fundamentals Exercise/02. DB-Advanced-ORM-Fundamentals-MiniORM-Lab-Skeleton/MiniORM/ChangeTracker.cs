...
namespace MiniORM
{
        ...
        where TEntity : class, new()
    {
        ...
        private static List<TEntity> CloneEntities(IEnumerable<TEntity> entities)
        {
            ...
            var propertiesToClone = typeof(TEntity).GetProperties().Where(pi => AllowedSqlTypes.SqlTypes.Contains(pi.PropertyType));
        ...
        {
            var monitoredProperties = typeof(TEntity).GetProperties().Where(pi => AllowedSqlTypes.SqlTypes.Contains(pi.PropertyType)).ToArray();

            var modifiedProperties = monitoredProperties
                .Where(pi => !Equals(pi.GetValue(clonedEntity), pi.GetValue(entity))).ToArray();
        ...
        private IEnumerable<object> GetPrimaryKeyValues(IEnumerable<PropertyInfo> primaryKeys, TEntity entity)
        {
