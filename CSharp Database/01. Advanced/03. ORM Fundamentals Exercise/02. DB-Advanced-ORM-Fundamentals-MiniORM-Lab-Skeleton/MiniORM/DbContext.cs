        ...
        public void SaveChanges()
        {
            ...
            foreach (IEnumerable<object> dbSet in dbSets)
            {
                ...
                using (new ConnectionManager(this.dbConnection))
                {
                    ...
                    {
                        foreach (IEnumerable<object> dbSetP in dbSets)
                        {
                            var dbSetType = dbSetP.GetType().GetGenericArguments().First();

                            var persistMetod = typeof(DbContext)
                                .GetMethod("Persist", BindingFlags.Instance | BindingFlags.NonPublic)
                                .MakeGenericMethod(dbSetType);

                            try
                            {
                                persistMetod.Invoke(this, new object[] { dbSetP });
                            }
