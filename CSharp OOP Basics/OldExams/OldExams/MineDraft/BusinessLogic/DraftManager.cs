...
public class DraftManager
{
    ...
    public string Day()
    {
        ...
        if (mode == "Full")
        {
            dayEnergyRequired = harvesters.Sum(h => h.EnergeyRequirement);
            dayMinedOre = harvesters.Sum(h => h.OreOutput);
        }
        else if (mode == "Half")
        {
            dayEnergyRequired = harvesters.Sum(h => h.EnergeyRequirement) * 0.6;
    ...
    public string Check(List<string> arguments)
    {
        ...
        Unit unit = 
            (Unit)harvesters.FirstOrDefault(h => h.Id == id) ??
