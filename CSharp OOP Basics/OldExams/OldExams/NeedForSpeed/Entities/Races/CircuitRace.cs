...
{
    protected CircuitRace(int length, string route, int prizePool, int laps) : base(length, route, prizePool)
    {
        this.Laps = laps;
    }
    ...
    public override string ToString()
    {
        ...
        if (this.Participants.Count > 0)
        {
            ...
            for (int i = 0; i < winners.Count; i++)
            {
                ...
                builder
                    .AppendLine($"{i + 1} {participantCar.Acceleration} {participantCar.Model} {performance}PP - ${GetPrize(i)}");
            }
        }
